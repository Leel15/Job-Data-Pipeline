...
def get_job_links(keyword=None, search_params=None):
    if search_params is None:
        search_params = SEARCH_PARAMS
    
    if keyword:
        print(f"\n🔍 جاري البحث عن وظائف: {keyword}")
    else:
        print("\nجاري البحث عن وظائف السعودية...")

    job_links = set()
    page = 1
    max_pages = 10  # حد أقصى للصفحات للبحث

    while len(job_links) < NUMBER_OF_JOBS and page <= max_pages:
        ...
        if not html:
            print(f"❌ لم يتمكن من تحميل الصفحة {page}")
            break
        ...
        print(
            f"✅ تم العثور على "
            f"{page_job_links} وظيفة في الصفحة {page}"
        )

        if page_job_links == 0:
            print("⚠️ لم يتم العثور على وظائف في هذه الصفحة. التوقف عن البحث.")
            break
    ...
    print(
        f"\n✅ تم العثور على إجمالي {len(job_links)} رابط وظيفة من تخصص: {keyword}"
    )
...
def scrape_tanqeeb_random_jobs():
    ...
    print("=" * 70)
    print("Tanqeeb Saudi - Tech Jobs - Multiple Pages")
    print("=" * 70)

    all_job_links = set()
    
    for keyword in TECH_KEYWORDS:
        links = get_job_links(keyword=keyword, search_params=SEARCH_PARAMS)
    ...
    output_path = (
        "data/raw/tanqeeb_saudi_jobs_tech.json"
    )
